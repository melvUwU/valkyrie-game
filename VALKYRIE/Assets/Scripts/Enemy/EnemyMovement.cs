...
public class EnemyMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        foreach (GameObject valkyrie in valkyries)
        {
            ...
            if (IsCollidingWithEnemy(valkyrie) && character != null)
            {
    ...
    {    // If the enemy isn't blocked and there are waypoints to follow
        if (!blocked && currentWaypoints.Count > 0)
        {
    ...
    {
        // Clear references to prevent accessing destroyed Transforms
        currentWaypoints.Clear();
        if (attackWith != null)
        {
            // Clear the reference to the Valkyrie
            attackWith = null;
        }
    }
